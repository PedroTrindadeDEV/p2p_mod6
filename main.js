...
const char_array = (text) =>
  clean_string(text)
    .replace(/[^a-zñáéíóú0-9]+/gim, "") // leave only letters
...
const word_array = (text) =>
  clean_string(text)
    .replace(/[\.:;?!,¿¡0-9]+/gm, "") //transforma signos de puntuación en espacios en blanco
    .split(" ")
    .filter((w) => w !== "");
...
const search_words = () => {
  ...
  let result = `Hay ${searched_words.length} palavras que contienen '${search}'.\n\n`;
